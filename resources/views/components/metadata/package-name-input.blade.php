...
>
    <div>
        <label for="{{$vendorParameter}}"
               class="block text-sm font-medium text-gray-700 dark:text-gray-400">
            {{Label::VENDOR}}
        </label>
        <input type="text" name="{{$vendorParameter}}" id="{{$vendorParameter}}"
               value="{{ $vendor }}"
               pattern="{{PackageName::VENDOR_REGEX}}"
               title="{{PackageName::HUMAN_READABLE_DESCRIPTION}}"
    ...
    <div class="auto-cols-auto">
        <label for="{{$projectParameter}}"
               class="block text-sm font-medium text-gray-700 dark:text-gray-400">
            {{ Label::PROJECT }}
        </label>
        <input
                type="text" name="{{$projectParameter}}"
                id="{{$projectParameter}}"
                pattern="{{PackageName::PACKAGE_REGEX}}"
                oninput="correctComposerNameOnInput(event)"
                value="{{ $project }}"
                title="{{PackageName::HUMAN_READABLE_DESCRIPTION}}"
