...
{{-- See https://getcomposer.org/doc/04-schema.md#name --}}
<div class="col-span-6 grid grid-cols-3" style="grid-template-columns: auto 2em auto;">
    <div>
        <label for="{{ $vendorParameter }}" class="block text-sm font-medium text-gray-700 dark:text-gray-400">
            {{ Label::VENDOR }}
        </label>
        <input type="text" name="{{ $vendorParameter }}" id="{{ $vendorParameter }}" value="{{ $vendor }}"
    ...
    <div class="auto-cols-auto">
        ...
        <input type="text" name="{{ $projectParameter }}" id="{{ $projectParameter }}"
            pattern="{{ \Domains\Composer\PackageName::PACKAGE_REGEX }}" oninput="correctComposerNameOnInput(event)"
            value="{{ $project }}" title="{{ \Domains\Composer\PackageName::HUMAN_READABLE_DESCRIPTION }}"
