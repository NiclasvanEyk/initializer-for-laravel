...
$backgroundClasses = $isEmptyOption
    ...
    : 'dark:bg-red-800 dark:bg-opacity-30 bg-red-100 bg-opacity-50';
...
>
    <div class="flex items-center h-5">
        <input type="radio" id="{{$id}}"
               ...
               class="mt-1 h-4 w-4 border-gray-300 {{$focusClasses}} transition"
               {{ $attributes }}
        />
    </div>
    <div class="ml-3 select-none w-full">
        ...
        >
            ...
            <span x-bind:class="{{$isEmptyOption ? 'false' : 'true'}} && {{$model}} === '{{$value}}' ? 'text-gray-700 dark:text-gray-400' : 'text-gray-500'" >
                {{ $slot }}
            </span>
        </span>
    </div>
